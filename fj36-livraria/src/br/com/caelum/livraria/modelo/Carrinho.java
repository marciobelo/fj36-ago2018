...
import br.com.caelum.estoque.soap.GetQuantidades;
import br.com.caelum.estoque.soap.GetQuantidadesResponse;
...
//		
//		this.itensDeCompra.stream().filter( ic -> ic.isImpresso())
//			.forEach( ic -> {
//				String cod = ic.getCodigo();
//				try {
//					ItemEstoque item = estoqueRmi.getItemEstoque(cod);
//					ic.setQuantidadeNoEstoque( item.getQuantidade());
...
//	}
	...
	public void verificarDisponibilidadeDosItensComSoap()
	{
		...
		GetQuantidades getQuantidades = new GetQuantidades();
		
		getQuantidades.getCodigos().addAll( codigos);
		
		GetQuantidadesResponse quantidades = estoqueWS.getQuantidades(getQuantidades, "TOKEN123");
		List<ItemEstoque> return1 = quantidades.getReturn();
		
		for( ItemEstoque item : return1)
		{
